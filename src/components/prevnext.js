...
export default ({ navigation }) => (
  <Row className="PageNavigation d-flex justify-content-between font-weight-bold">
    {navigation.prev ? (
      <Link className="prev d-block col-md-6" to={navigation.prev.slug}>
        {' '}
        &laquo; {navigation.prev.title}
      </Link>
    ) : (
      <span className="prev d-block col-md-6" />
    )}
    {navigation.next ? (
      <Link
        className="next d-block col-md-6 text-lg-right text-right"
